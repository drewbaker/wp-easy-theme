...
class Router
{
    ...
    private function locate(): void
    {
        ...
        add_filter('template_include', function ($template) use ($folder, $query_var, $routes) {
            ...
            if (isset($routes[$name]['template'])) {
                $name = $routes[$name]['template'];
            }
            ...
            // Attempt to load the template's SCSS file
            $scss_abs_path = str_replace('.php', '.scss', $template);
            $css_abs_path = str_replace('.php', '.css', $template);
            if (file_exists($scss_abs_path)) {
                $scss_uri = get_template_directory_uri() . '/' . $folder . '/' . $name . '.scss';
                wp_enqueue_style($name, $scss_uri, [], null, 'all');
            }
            if (file_exists($css_abs_path)) {
                $css_uri = get_template_directory_uri() . '/' . $folder . '/' . $name . '.css';
